...
from python_algo_production.signal.euro_filter import *
...
def test_one_euro_filter_step():
    print(__file__ + " start!!")

    # create the filter
    f_cmin = 1.0  # Intercept f_cmin in Equaiton 7
    beta = 0.0  # Slope β shown in Equation 7
    one_euro_filter = OneEuroFilter(x0, f_cmin, beta, rate)

    if show_animation:
        axes[0].cla()
        axes[0].set_title("One Euro Filter Step Test")
    ...
    for i in range(len(x_list)):
        ...
        x_hat = one_euro_filter.filter_step(x)

        if show_animation:
            t_disp.append(t_list[i])
            x_disp.append(x)
            ...
            print(f"Time:{t:.2f}, Value:{x:.2f} -> Filtered Value:{one_euro_filter.x_hat:.2f}")
...
def test_one_euro_filter():
    ...
    if show_animation:
        axes[1].cla()
        axes[1].set_title("One Euro Filter Test")
    ...
    # apply the filter
    x_hat_array = one_euro_filter.filter(x_array)
...
def test_one_euro_filter_params():
    print(__file__ + " start!!")

    tuples = ([0.5, 0], [0.5, 0.25], [1.0, 0], [1.0, 0.25], [2.0, 0], [2.0, 0.25])
    fc_min, beta = zip(*tuples)  # unzip the list of tuples using * operator

    # create the filters with different params: the actual cos signal has a frquency of 1.0Hz, let's try 0.5 and 2.0 for fc_min
    one_euro_filter1 = OneEuroFilter(x0, fc_min[0], beta[0], rate)  #     one_euro_filter = OneEuroFilter(x0, f_cmin, beta, rate)
    one_euro_filter2 = OneEuroFilter(x0, fc_min[1], beta[1], rate)
    one_euro_filter3 = OneEuroFilter(x0, fc_min[2], beta[2], rate)  #     one_euro_filter = OneEuroFilter(x0, f_cmin, beta, rate)
    one_euro_filter4 = OneEuroFilter(x0, fc_min[3], beta[3], rate)
    one_euro_filter5 = OneEuroFilter(x0, fc_min[4], beta[4], rate)
    one_euro_filter6 = OneEuroFilter(x0, fc_min[5], beta[5], rate)
    ...
    param_fig, param_axes = plt.subplots(len(fc_min), figsize=(10, 24))

    if show_animation:
        t_disp = []
        x_disp = []
        x_hats_disp =[[] for _ in range(len(fc_min))]
        x_true_disp = []
        for i in range(len(fc_min)):
            param_axes[i].cla()
            param_axes[i].set_title(f"One Euro Filter with f_cmin={fc_min[i]} and β={beta[i]}")
    ...
    # apply the filters
    x_hats_array = [np.array([]) for _ in range(len(fc_min))]
    for i in range(len(fc_min)):
        x_hats_array[i] = one_euro_filters[i].filter(x_array)
...
def test_two_euro_filter_step():
    print(__file__ + " start!!")

    # create the filter
    f_cmin = 1.0  # Intercept f_cmin in Equaiton 7
    beta = 0.0  # Slope β shown in Equation 7
    two_euro_filter = TwoEuroFilter(x0, y0, f_cmin, beta, rate)

    axes3d[0].cla()
    axes3d[0].set_title("Two Eueo Filter Step Test")
    ...
    for i in range(len(t_list_xy)):
        ...
        xy_hat = two_euro_filter.filter_step(xy)
...
def test_two_euro_filter():
    ...
    # apply the filter
    xy_hat_array = two_euro_filter.filter(xy_array)
